...
describe('ランキング機能', () => {
  describe('基本機能テスト', () => {
    test('ランキングエントリの型定義が正しい', () => {
      ...
      // ランクの妥当性チェック
      expect(['SS', 'S', 'A', 'B', 'C', 'D', 'F']).toContain(mockRanking.rank);
    });
    ...
    test('全ランク（SS、S、A、B、C、D、F）の妥当性チェック', () => {
      const allRanks = ['SS', 'S', 'A', 'B', 'C', 'D', 'F'];
      ...
      for (const rank of allRanks) {
        const mockRanking: RankingEntry = {
          ...
          username: 'テストユーザー',
          score: 8000,
          rank: rank as 'SS' | 'S' | 'A' | 'B' | 'C' | 'D' | 'F',
          created_at: '2025-01-15T10:00:00.000Z',
        };

        expect(allRanks).toContain(mockRanking.rank);
      }
    });
  });
});
